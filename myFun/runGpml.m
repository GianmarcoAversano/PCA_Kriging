...
parfor ii = 1 : y_dim
    if iscell(hyp)
        [mu_ii, s2_ii, hyp2_ii, fmu_ii, fs2_ii, lp_ii, post_ii] = GPR(x, y(:,ii), xs, options, hyp{ii});
    else
        [mu_ii, s2_ii, hyp2_ii, fmu_ii, fs2_ii, lp_ii, post_ii] = GPR(x, y(:,ii), xs, options, hyp);
    end
...
function [mu, s2, hyp2, fmu, fs2, lp, post] = GPR(x, y, xs, options, hyp)
n_points = size(xs,1);
