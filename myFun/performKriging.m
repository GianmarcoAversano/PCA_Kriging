...
is_parallel = true;
if n_args > 1
    is_parallel = varargin{2};
end
if isempty(is_parallel) || (is_parallel ~= 1 && is_parallel ~= 0)
    is_parallel = true;
end
% Generate starting guess for the hyper-parameters
if isempty(guess)
    fprintf('Generating hyper-parameters...\n');
    guess = generateHyperparameters0(samples, values, trendFun, corrFun, false);
    fprintf('Hyper-parameters generated.\n');
end
% We need to perform Kriging one scalar at time.
y_dim = size(values, 1);
n_points = size(prediction_points, 1);
if n_points > 0
    ...
    if is_parallel
        parfor ii = 1 : y_dim
            if ~iscell(guess)
                [m, s, kr] = ood_level1(samples, values(ii,:), ...
                    prediction_points, trendFun, corrFun, guess);
            else
                [m, s, kr] = ood_level1(samples, values(ii,:), ...
                    prediction_points, trendFun, corrFun, guess{ii});
            end
            predictions(ii,:) = m;
            mse(ii,:) = s;
            k{ii} = kr;
    ...
    if is_parallel
        parfor ii = 1 : y_dim
            if ~iscell(guess)
                [~, ~, kr] = ood_level1(samples, values(ii,:), ...
                    prediction_points, trendFun, corrFun, guess);
            else
                [~, ~, kr] = ood_level1(samples, values(ii,:), ...
                    prediction_points, trendFun, corrFun, guess{ii});
