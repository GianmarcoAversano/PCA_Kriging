...
if ~isempty(y_cpca)
    hold on;
    plot(y_data/norm_fac, y_cpca/norm_fac, 'Marker',Markers{2}, 'Color','k', 'LineStyle','none'); 
    err = get_err(y_data, y_cpca);
    leg{end+1} = ['cpca ', s];
end
if ~isempty(y_lpca)
    hold on;
    plot(y_data/norm_fac, y_lpca/norm_fac, 'Marker',Markers{3}, 'Color','r', 'LineStyle','none');
    err = get_err(y_data, y_lpca);
    leg{end+1} = ['lpca ', s];
end
if ~isempty(y_lcpca)
    hold on;
    plot(y_data/norm_fac, y_lcpca/norm_fac, 'Marker',Markers{4}, 'Color','m', 'LineStyle','none'); 
    err = get_err(y_data, y_lcpca);
    leg{end+1} = ['lcpca ', s];
end
if ~isempty(y_direct)
    hold on;
    plot(y_data/norm_fac, y_direct/norm_fac, 'Marker',Markers{5}, 'Color','g', 'LineStyle','none'); 
    err = get_err(y_data, y_direct);
    leg{end+1} = ['direct ', s];
end
% Plot x = y line
plot([min(y_data) max(y_data)]/norm_fac, [min(y_data) max(y_data)]/norm_fac, 'k--'); 
...
if norm_fac ~= 1
    ...
    if ~boolean_var
        ylabel('Recovered value ', 'Interpreter','LaTex');
    else
